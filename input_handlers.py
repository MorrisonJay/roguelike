...
class LevelUpEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.console.Console) -> None:
        ...
        console.print(
            x=x + 1,
            y=5,
            string=f"b) Strength (+1 to attack, from {self.engine.player.fighter.power})",
        )
        console.print(
            x=x + 1,
            y=6,
    ...
    def ev_mousebuttondown(
        ...
        """Don't let the player click to exit the menu, like normal"""
        return None
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if key in MOVE_KEYS:
            ...
            if event.mod & (tcod.event.Modifier.LALT | tcod.event.Modifier.RALT):
                modifier *= 20
            elif event.mod & (tcod.event.Modifier.LCTRL | tcod.event.Modifier.RCTRL):
                modifier *= 10
            elif event.mod & (tcod.event.Modifier.LSHIFT | tcod.event.Modifier.RSHIFT):
                modifier *= 5
            ...
            x += dx * modifier
            y += dy * modifier
            # Clamp the cursor index top the map size.
...
class HistoryViewer(EventHandler):
    ...
    def on_render(self, console: tcod.console.Console) -> None:
        ...
        log_console.print(
            x=0, y=0, width=log_console.width, height=1, text="{Message history}", alignment=tcod.constants.CENTER
        )
